...
def _get_ip_prefix(server_network, net_mask):
    ip_prefix = "".join(
        [
            "{}.".format(x)
            for (x, y) in zip(server_network.split("."), net_mask.split("."))
            if y != "0"
        ]
    )
...
def fill_server_values(
    key_dir, server_network, server_port_in, config_path, server_proto, server_mask, cipher
):
    ...
    server_file_contents = (
        server_file_template.render(
            ...
            contents_ca_crt=contents_ca_crt,
            contents_server_crt=contents_server_crt,
            contents_server_key=contents_server_key,
            contents_dh2048=contents_dh2048,
...
def fill_client_values(
    dns_address, key_dir, client_name, server_port_out, server_aliases, server_proto, cipher
):
    """
    Args:
        dns_address:
    ...
    Returns: A tuple with four distinct strings corresponding to
             client profles: linux, linux-redirect, windows, windows-redirect.
    """
    ...
    client_windows_redir = (
        file_header
        + "# WINDOWS/ANDROID profile (redirect)\n"
        + client_std
        + redir_string
        + "block-outside-dns\n"
        + dns_line
...
def get_all_clients_by_keyfiles(key_dir):
    ...
    existing_clients = [
        y.split(".")[0] for y in [os.path.basename(x) for x in key_files] if y not in ignore_files
    ]
    return existing_clients


#%% File-writing functions
def write_single_client_profiles(
    output_dir,
    vpn_name,
    dns_address,
    key_dir,
    ...
    client_l, client_lr, client_w, client_wr = fill_client_values(
        dns_address, key_dir, client_name, server_port_out, server_aliases, server_proto, cipher
    )
...
def write_server_config(
    ...
    config_file_contents = fill_server_values(
        key_dir, server_network, server_port_in, config_path, server_proto, net_mask, cipher
    )
...
def write_server_ipp_file(
    vpn_name, client_file, dir_name, output_dir, key_dir, server_network, net_mask
):
    if client_file is None:
        print(
            "[{}] No value for 'CLIENT_LIST' was specified.  Won't write ipp.txt...".format(
                vpn_name
            )
        )
        return
    ...
    clients_addr_but_not_existing = {
        x: client_ip_dict[x] for x in client_ip_dict if x not in clients_existing
    }

    ip_prefix = _get_ip_prefix(server_network, net_mask)
    static_ips = "## ipp.txt for {} ({})\n## certificate_client_name,ip_address\n".format(
        vpn_name, server_network
    )
    ...
    if len(clients_existing_but_no_addr) > 0:
        msg_not_in_client_file = (
            "Profiles NOT added to ipp.txt "
    ...
    if len(clients_addr_but_not_existing) > 0:
        msg_no_key = "Profiles NOT written (entries found in {}, but no key-pair found)".format(
            client_list_full_path
        )
